...
#' dct01['color']<-NA
#' <div style="background:yellow">
#' * In addition to the comments it would be better to include the actual code
#' without comments, so that it runs. Like this:
dct01['color'] <- NA
#' * The above doesn't really address the question. To get the fraction missing
#' (`NA`) for each column in `dat01` we need to...
#'    * ...take each column
dct01$column;
#'    * ...subset that column from `dat01`:
dat01[,dct01$column];
#'    * ...run `is.na()` on it. If you were working with just one vector it would 
#'   look like `is.na(FOO)`. To do it to each column of a `data.frame` you need
#'   to give `is.na` as an argument to `sapply()`:
sapply(dat01[,dct01$column],is.na)                   %>% head()
#' *I am piping it to `head()` so that it shows only the first few lines instead of
#' filling up your screen, that part is optional.*
#' 
#'    * Now you can run `apply()` on the above results again but this time
#'    using `mean` as the second argument. We have to use `simplify=F` on the
#'    inner `sapply()` so that its output will remain as a `list` so that the
#'    outer `sapply()` can work on it.
sapply(sapply(dat01[,dct01$column],is.na,simplify = F),mean)
#'    The following is equivalent:
# sapply(dat01[,dct01$column],is.na) %>% sapply(mean) %>% head()
#' *Or you could use `colMeans()`, look it up.*
#' 
#'    * Finally you assign the resulting vector of fractions to a new column in
#'    `dct01`
dct01$fraction_missing <- sapply(sapply(dat01[,dct01$column],is.na,simplify=F),mean);
#' </div>
#' View(dct01 )
#' unique (unlist (lapply (dct01, function (x) which (is.na (x)))))
#' <div style="background:yellow">
#' Not quite. This is why you need both code and comments in your answer, not just
#' comments. The number of unique values column is supposed to be a separate one
#' from the fraction of missing values, and they need to be from `dat01` not 
#' `dct01` itself. `dct01` is just the data dictionary. You are right that `unique()`
#' is part of the answer, but it then needs to be enclosed in `length()` and that
#' will give the number of unique values for each column:
dct01$number_unique <- sapply(sapply(dat01[,dct01$column],unique,simplify=F),length);
#' </div>
#'
#' ## 2. Pattern Matching
#' 
...
#' <div style="background:yellow">
#' If you had run the code above you would see the following:
grep("_date", dct01, value = T);
#' This is not a vector of TRUE/FALSE values and you did not assign it to a column
#' in `dct01`. The simplest way to get TRUE/FALSE is to use `grepl()` instead of
#' `grep()` and you should run it on a specific column. Right now it's returning
#' one long string, not a vector of column names.
grepl("_date",dct01$column);
dct01$analyzable <- grepl("_date",dct01$column);
#' Except we need it to say `FALSE` for the columns that match instead of `TRUE`.
...
#' <div style="background:yellow">
#' Running the separate commands one after the other does not accomplish the goal
#' of this question. You can either make a really long expression where each of
#' the above is fixed like in the first example I gave and then separated from
#' the next one by a `&` (the logical 'and' operator). But it's easier to just
#' put them all together into one pattern like we did in class. Within a grep
#' pattern the logical operator is `|` (grep is not R, it's a purpose-specific
#' language for searching through text that happens to be available for our use
#' within R).
dct01$analyzable <- !grepl("_date|_unit|_info|patient",dct01$column);
dct01[,c('column','analyzable')]
#' To determine which columns are of type `character` we could use `sapply()` with
#' `class` as its second argument and put ` == 'character'` after it. But we 
#' already have a `dct01$char` column from class, so we can use `&` to combine
#' it with the above `analyzable` column to answer the second part of this question.
dct01$analyzable_character <- dct01$char & dct01$analyzable;
dct01[,c('column','char','analyzable','analyzable_character')];
#' </div>
#' 
