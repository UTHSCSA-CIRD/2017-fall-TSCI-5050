...
#' Thank you for taking this class. I have reviewed this homework and given
#' it a passing grade. Please see my detailed feedback below. In the HTML
#' version it will be highlighted in yellow.
#' </div>
#' 
...
#' 
#' <div style="background:yellow">
#' Below I fix some of the formatting in addition to responding to your comments.
#' To see what I changed, in the bash (not R) console you could type 
#' `diff KEI_TSCI5050_HW02.R graded_KEI_TSCI5050_HW02.R`. 
#' </div>
#' 
...
#' <div style="background:yellow">
#' Try it in separate pieces like I do in class-- you'll see that `is.na(FOO)` 
#' returns a matrix of TRUE/FALSE values with the same number of rows and columns 
#' as the original data.frame. Then when you use the result as an argument to 
#' `mean()` you get one single number-- the mean of all the `TRUE` (or `1`)  and 
#' `FALSE` or (`0`) values. Sometimes that's useful, like if you wanted to know 
#' the fraction of the entire data.frame that is missing (37%). But you want a 
#' separate fraction for each column. Since you just have one number and are
#' assigning it to a column with multiple rows, this number just gets copied to
#' each row. Again, in other circumstances this is really useful-- like when you
#' want to fill in an identical default value all the way down the column without 
#' having to type or copy-paste for each row.
#' 
#' </div>
#' sum(is.na(FOO))
...
#' <div style="background:yellow">
#' ...and to get counts of unique values you would use 
#' `sapply(FOO,function(xx) length(unique(xx))`. But yes, the second one,
#'  `mean(is.na(xx))` is correct.
#' 
#' Yes, you got it! `sapply()` takes not a function result but a function 
#' itself as one of its arguments (the one that says what it is we want to 
#' apply). There are two main ways to do that: either give it just the name
#' of the function without the parentheses (one of the few exceptions to the
#' that functions should always be written with parentheses). The other is what
#' you did here -- create a temporary un-named function. There is more on that
#' in the original hints bullets-list above.
#' </div>
...
#' 
#' ## Answer: 
#' from prior run.R class
#' 
# grep('^v',dct01[ !dct01$meta & dct01$char , 'column' ],val=T)] %>% sapply(function(xx) !is.na(xx),simplify=F)

#' `$`` matches end of string

FOO <- !grepl('_date$|_unit$|_info$|_patient$', dct01$column)
dct01$non_analytic <- FOO
#' <div style="background:yellow">
#' Good. Except `_date$|_unit$|_info$|_patient$` will find the columns that are
#' *non* analytic so when you negated the query with a `!` you got `TRUE` values
#' for every *analytic* column. Lets create a column  with a less confusing name
#' and copy this result to it.
dct01$analytic <- dct01$non_analytic;
dct01[,c('column','analytic')] %>% head()
#' </div>

# grepl ('_date$|_unit$|_info$|_patient$'%>%"_class$',dct01$class) = WRONG
#' <div style="background:yellow">
#' The error comes from your command being interrupted by a `'%>%"`. That single
#' quotation mark is the first one after the one that opened this quote, so 
#' R tries and fails to treat the `%>%"` that comes after as though it's part of
#' some command. Also, you were looking in the wrong column-- `dct01$class` instead
#' of `dct01$names`. To see what values class contains you could do `summary(dct01$class)`
#' </div>
BAY <- grepl ("TRUE", dct01$char)
#' <div style="background:yellow">
#' The above works but is redundant-- you get a T/F vector that tells you whether 
#' `dct01$char` is true or false... but `dct01$char` already is a T/F vector so
#' why bother?
identical(grepl('TRUE',dct01$char),   dct01$char  );
#' </div>
# grepl ('_date$|_unit$|_info$|_patient$'%>%"_class$',dct01) = WRONG
# if (FOO)%>%(BAY), else (FALSE) = WRONG
# <div style="background:yellow">
#' But if you already have vectors that consist of T and F values you never need 
#' to use `grepl()` or `if(...) {...} else {...}` or anything else fancy-- you 
#' can just combine those vectors in whatever way you need with AND (`&`) or OR 
#' (`|`).
#' </div>
BAZ <- if (FOO) {dct01$char==TRUE} else {FALSE}
dct01$HW_2 <- BAZ
#' <div style="background:yellow">
#' This does something but not what you think. The `if (FOO)` expects to have 
#' only one TRUE/FALSE answer. If `FOO` is `TRUE` then it will execute whatever 
#' statement is in the first group of brackets..which as per above explanation
#' amounts to an exact copy of the `dct01$char` column. 
dct01$char==TRUE
dct01$char
#' If `FOO` is `FALSE` then whatever is in the second pair of brackets gets
#' executed. In this case a literal `FALSE` and that's it.
#' 
#' But `if()` needs to make *one* TRUE/FALSE decision and yet you are giving it
#' a vector of 19 different values. So it takes only the first one and decides
#' using that. In this case it's a `TRUE` so `dct01$HW_2` is always set to be an 
#' exact copy of the `dct01$char` column. But if the order was different and the
#' first value of `FOO` had been `FALSE` then the `dct01$HW_2` column would always
#' be filled with the `FALSE` from the second column.
#' 
#' But how *to* find the columns that are both analytic and character? By using
#' the `&` operator:
#' 
dct01$character_analytic <- dct01$char & dct01$analytic
