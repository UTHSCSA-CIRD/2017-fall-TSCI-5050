...
#' Superb work, Jeffrey! You really got what I have been trying to teach, well
#' done. Please see my detailed feedback below. In the HTML
...
tail(dct01)
#' <div style="background:yellow">
#' This definitely gets the job done. Good use of `tail()` and `head()` below. 
#' But the way you did it, you don't need `sum()`-- `length()` already returns
#' the number of missing values and `sum()` just passed it through unaltered.
identical(sapply(dat01,function(xx) sum(is.na(xx))),sapply(dat01, function(xx) length(which(is.na(xx)))));
#' ...and then divide by `nrow(dct01)` to get the fraction missing. Or you could
#' instead get rid of `which()` and `length()` and put `sum()` back in to directly
#' count up the number (because each `TRUE` contributes a `1` to the sum and each 
#' `FALSE` a `0`)
identical(sapply(dat01,function(xx) sum(is.na(xx))),sapply(dat01, function(xx) sum(is.na(xx))))
dct01$fraction_missing <- sapply(dat01, function(xx) sum(is.na(xx))/nrow(dat01));
#' Instead of dividing by `nrow(dat01)` you could use `mean()` instead of `sum()` and 
#' save one more step:
identical(sapply(dat01, function(xx) sum(is.na(xx)))/nrow(dat01),sapply(dat01, function(xx) mean(is.na(xx))))
dct01$fraction_missing <- sapply(dat01,function(xx) mean(is.na(xx)));
#' There is even a shorter way: `colMeans()`. But it's specifically for means, most
#' other aggregation tasks we have to rely on general purpose tools such as `sapply()`.
#' and write our own functions like you did (did I mention, great job?).
identical(sapply(dat01, function(xx) sum(is.na(xx)))/nrow(dat01),colMeans(is.na(dat01)))
dct01$fraction_missing <- colMeans(is.na(dat01));
#' Notice that you can just directly pass a data.frame to `is.na()` and R
#' interprets it in the smartest way possible of instead of giving you
#' some lazy error? That's R for you. :-)
#' </div>
#' > Unique values
...
head(dct01)
#' <div style="background:yellow">
#' As above, `sum()` is extraneous, but otherwise perfect answer.
#' </div>
...
head(dct01)
#' <div style="background:yellow">
#' Good. But you could do this in one step by negating (`!`) the T/F output from `grepl()`
identical(dct01$analytic,!grepl('_unit$|_info$|_date$|patient',dct01$column));
#' The two are the same.
#' </div>
dct01$analytic2 <- F
dct01[grepl('character', dct01$class),'analytic2'] <- T
#' <div style="background:yellow">
#' This works but when an exact match is known regular expressions are not recommended
#' because they are slower and add complexity to the code. An exact match is done
#' with `==` for character strings just like for numbers.
identical(grepl('character',dct01$class),   dct01$class == 'character');
#' </div>
dct01[grepl('FALSE', dct01$analytic), 'analytic2'] <- F
tail(dct01)
#' <div style="background:yellow">
#' The above is doubly true for things that are *already* logical vectors. Just
#' use logical '&' .
identical(dct01$analytic2, dct01$analytic & dct01$class=='character');
#' </div>
